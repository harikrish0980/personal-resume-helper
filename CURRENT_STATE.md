...
- Local resume profiles under `Career-Ops/profiles`, with `Resume 1` as the default profile.
...
- Scanner Inbox is active for Career-Ops pipeline/API results; fresh direct ATS rows are prioritized and stale/history/search rows are separated by filters.
...
- The 2026-05-15 stabilization pass intentionally avoids changing resume templates, Career-Ops output format, or PDF generation internals.
...
- 2026-05-18 resume/run polish:
  ...
  - Resume generation still reads the full `Career-Ops/article-digest.md`, not the preview text.
...
- 2026-05-18 multi-profile resume support:
  ...
  - Each profile reads its own `cv.md` and `article-digest.md` under `Career-Ops/profiles/<profile>/`.
...
- 2026-06-25 scanner inbox integration:
  ...
  - Scanner Inbox reads `Career-Ops/data/pipeline.md` and enriches rows with `data/scan-history.tsv` when available.
...
- 2026-06-26 scanner quality fix:
  ...
  - Direct scanner sources expanded from 1 to 22 API-detectable Greenhouse/Ashby/Lever companies in `Career-Ops/portals.yml`.
...
- 2026-06-29 scanner inbox verification:
  ...
  - Old Career-Ops pipeline rows are still recoverable through filters, but they no longer dominate the default working list.
...
- 2026-06-29 cover letter and tailoring polish:
  ...
  - The app still keeps cover letters local under Career-Ops output folders and keeps final application submission manual.
## Standard Check
